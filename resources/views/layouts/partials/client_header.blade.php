{{-- resources/views/layouts/partials/client_header.blade.php (v3.6 · logo proporcional + sin botón menú) --}}
@php
  ...
  try {
      ...
      if (is_file($p)) {
        ...
        if ($raw && preg_match('~<svg[^>]*>(.*)</svg>~si', $raw, $m)) $spriteInline = trim($m[1]);
      }
  } catch (\Throwable $e) {}
  ...
  // Configuración (fallback a perfil si no existe)
  $rtSettings = Route::has('cliente.settings')
      ? route('cliente.settings')
      : (Route::has('cliente.configuracion') ? route('cliente.configuracion') : $rtPerfil);
  ...
  <div class="top-actions" style="display:flex; align-items:center; gap:14px;">
    ...
    <div class="account" style="display:flex; align-items:center; gap:10px;">
      <div class="acct-meta">
        ...
        <span class="acct-plan">{{ $plan === 'PRO' ? 'PRO' : 'FREE' }}</span>
      </div>

      <div class="menu-profile" style="position:relative">
        <button id="btnProfile" class="btn icon" aria-haspopup="menu" aria-expanded="false" aria-controls="menuProfile" title="Cuenta">
        ...
        <div id="menuProfile" class="dropdown" role="menu" aria-labelledby="btnProfile" hidden>
          ...
          <form action="{{ $rtLogout }}" method="POST" role="none">@csrf
...
<style>
  ...
  .topbar, .topbar * { font-family: 'Poppins', var(--font-sans, ui-sans-serif), system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif; }
  .topbar{
  ...
  html.theme-dark .topbar{ background: color-mix(in oklab, #0b1220 86%, transparent); border-bottom-color: rgba(255,255,255,.12); }

  .ico{ width:var(--ico); height:var(--ico); display:block; }

  .searchbar{
    display:flex; align-items:center; gap:8px;
    border:1px solid var(--bd,#e5e7eb); border-radius:999px; background: var(--chip, #f8fafc);
    padding:0 12px; height:40px; max-width:620px; width:100%;
  }
  html.theme-dark .searchbar{ background: color-mix(in oklab, #fff 6%, transparent) }
  .searchbar .ico{ color: color-mix(in oklab, var(--text,#0f172a) 60%, transparent) }
  .searchbar input{ all:unset; flex:1; font-weight:600; color:var(--text,#0f172a); font-size:14px; }

  .btn.icon{
    position:relative; display:inline-flex; align-items:center; justify-content:center;
    width:40px; height:40px; border-radius:12px; border:1px solid var(--bd,#e5e7eb); background:var(--card,#fff); cursor:pointer;
  }
  html.theme-dark .btn.icon{ background: color-mix(in oklab, #fff 6%, transparent); border-color: rgba(255,255,255,.12) }
  .btn.icon .ico{ color: var(--text,#0f172a) }

  .badge{
    position:absolute; top:-6px; right:-6px; min-width:18px; height:18px; padding:0 6px; border-radius:999px; background:#ef4444; color:#fff;
    font:800 11px/18px system-ui; text-align:center; border:2px solid color-mix(in oklab, var(--card,#fff) 92%, transparent);
  }

  .acct-meta{ display:flex; flex-direction:column; align-items:flex-end; line-height:1.1; min-width:160px; }
  ...
  .dropdown{
    ...
    color:var(--text);                  /* antes: var(--ink,#0f172a) */
  ...
  html.theme-dark .dropdown{ background:#0b1220; border-color:#2b2f36 }

  .dropdown a, .dropdown button{
    text-align:left; text-decoration:none; color:inherit; background:transparent; border:0; cursor:pointer;
    padding:10px 12px; border-radius:10px; font-weight:600;
  }
  .dropdown a:hover, .dropdown button:hover{ background:rgba(0,0,0,.06) }
  html.theme-dark .dropdown{ background:#0b1220; color:#e5e7eb; border-color:#2b2f36 }
  html.theme-dark .dropdown a:hover, html.theme-dark .dropdown button:hover{ background:rgba(255,255,255,.08) }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function closeProfile(){ if(menuProfile){ menuProfile.hidden = true; btnProfile?.setAttribute('aria-expanded','false'); } }
  btnProfile?.addEventListener('click', ()=>{ if(!menuProfile) return; const open = menuProfile.hidden; menuProfile.hidden = !open; btnProfile.setAttribute('aria-expanded', open ? 'true' : 'false'); });
  document.addEventListener('click', (e)=>{ if(menuProfile?.hidden) return; if(e.target.closest('.menu-profile')) return; closeProfile(); });
