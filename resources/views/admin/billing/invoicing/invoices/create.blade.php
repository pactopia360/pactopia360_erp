    ...
    <div class="invx-main">
        <section class="invx-card invx-hero">
            ...
            <div class="invx-hero__actions">
                ...
                <button type="button" class="invx-btnx invx-btnx--primary" id="invxOpenAiPanelTop">IA rápida</button>
            </div>
        </section>
        ...
        <form method="POST" action="{{ $routeStoreOne }}" enctype="multipart/form-data" class="invx-form" id="invxAdvancedManualForm">
            ...
            <input type="hidden" id="invx_wizard_step" value="1">
            <input type="hidden" id="invx_clone_mode" value="skip">
                    ...
                    <div class="invx-block__body">
                        <div class="invx-files">
                            ...
                            <div class="invx-filecard">
                                ...
                                <input id="manual_xml_copy" type="file" class="invx-input" accept=".xml,application/xml,text/xml,.txt,text/plain">
                            </div>
                        </div>
                    </div>
                </section>
...
(function () {
    ...
    const STEP_META = {
        1: { title: 'Paso 1 · Tipo de comprobante', sub: 'Selecciona el tipo de comprobante y arranca el flujo.' },
        2: { title: 'Paso 2 · Clonado o captura desde cero', sub: 'Decide si usarás XML como base o si vas a omitir este paso.' },
        3: { title: 'Paso 3 · Datos generales', sub: 'Captura cuenta, periodo, monto y fecha base de la factura.' },
        4: { title: 'Paso 4 · Emisor y receptor', sub: 'Busca y selecciona las partes fiscales.' },
        5: { title: 'Paso 5 · Datos SAT', sub: 'Completa configuración fiscal y revisa método/forma de pago.' },
        6: { title: 'Paso 6 · Adjuntos base', sub: 'Carga archivos base para registrar esta factura.' },
        7: { title: 'Paso 7 · Revisión final', sub: 'Valida el resumen y genera la factura.' }
    };

    const TIPO_HELP = {
        I: 'Ingreso: ideal para facturas por venta de productos o prestación de servicios. La IA sugerirá configuraciones típicas de cobro.',
        E: 'Egreso: útil para notas de crédito, devoluciones o descuentos. La IA te ayudará a detectar datos relacionados.',
        P: 'Pago: este tipo se usa para complementos de pago. Más adelante aquí conectaremos el flujo de parcialidades y administración de pagos.',
        N: 'Nómina: enfocado a recibos de nómina. La IA podrá adaptar recomendaciones fiscales específicas.',
        T: 'Traslado: se usa para traslado de mercancías. La IA podrá sugerir complementos como Carta Porte cuando aplique.'
    };
    ...
    function setStep(step) {
            ...
            qsa('[data-step-pane]').forEach((pane) => {
                pane.classList.toggle('is-active', parseInt(pane.getAttribute('data-step-pane'), 10) === currentStep);
            });

            qsa('.invx-stepdot').forEach((dot) => {
                const dotStep = parseInt(dot.getAttribute('data-step-target'), 10);
                const completed = isStepCompleted(dotStep);

                dot.classList.toggle('is-active', dotStep === currentStep);
                dot.classList.toggle('is-done', completed && dotStep !== currentStep);
            });
            ...
            if (hiddenStep) hiddenStep.value = String(currentStep);
            if (prevBtn) prevBtn.disabled = currentStep <= 1;

            if (nextBtn) nextBtn.classList.toggle('invx-hidden', currentStep >= totalSteps);
            if (submitBtn) submitBtn.classList.toggle('invx-hidden', currentStep < totalSteps);

            if (navTitle) navTitle.textContent = STEP_META[currentStep].title;
            if (navSub) navSub.textContent = STEP_META[currentStep].sub;
        }

        function isStepCompleted(step) {
        const tipo = qs('#invx_tipo_comprobante');
        const account = qs('#manual_account_id');
        const period = qs('#manual_period');
        const amount = qs('#manual_amount_mxn');
        const emisor = qs('#invx_emisor_id');
        const receptor = qs('#invx_receptor_id');
        const metodo = qs('#invx_metodo_pago');
        const moneda = qs('#invx_moneda');
        const pdf = qs('#manual_pdf');
        const xml = qs('#manual_xml');

        if (step === 1) {
            return !!(tipo && String(tipo.value || '').trim() !== '');
        }
        ...
        if (step === 3) {
            return !!(
                account && String(account.value || '').trim() !== '' &&
        ...
        if (step === 4) {
            return !!(
                emisor && String(emisor.value || '').trim() !== '' &&
                receptor && String(receptor.value || '').trim() !== ''
            );
        }
        ...
        if (step === 5) {
            return !!(
                ...
                moneda && String(moneda.value || '').trim() !== ''
            );
        }

        if (step === 6) {
            const hasPdf = !!(pdf && pdf.files && pdf.files.length);
            const hasXml = !!(xml && xml.files && xml.files.length);
            return hasPdf || hasXml;
        }

        if (step === 7) {
            return isStepCompleted(1) && isStepCompleted(3) && isStepCompleted(4) && isStepCompleted(5) && isStepCompleted(6);
        }
    ...
    function bindTypeButtons() {
        const select = qs('#invx_tipo_comprobante');
        const buttons = qsa('.invx-typebtn');
        const tipoLabel = qs('#invxTipoActualLabel');
        ...
        select.addEventListener('change', function () {
            ...
            if (tipoLabel) tipoLabel.textContent = text;
            if (tipoSmartText) tipoSmartText.textContent = TIPO_HELP[val] || TIPO_HELP.I;

            const summaryTipo = qs('#invxSummaryTipo');
            const reviewTipo = qs('#invxReviewTipo');

            if (summaryTipo) summaryTipo.textContent = text;
            if (reviewTipo) reviewTipo.textContent = text;
        });

        select.dispatchEvent(new Event('change', { bubbles: true }));
    }

    function bindCloneMode() {
        const cloneInput = qs('#invx_clone_mode');
        const label = qs('#invxCloneModeLabel');
        const text = qs('#invxCloneModeText');
        const review = qs('#invxReviewClonado');
        const useBtn = qs('#invxUseCloneMode');
        const skipBtn = qs('#invxSkipCloneMode');

        function apply(mode) {
            if (cloneInput) cloneInput.value = mode;

            if (mode === 'clone') {
                if (label) label.textContent = 'Clonar desde XML';
                if (text) text.textContent = 'Se intentará usar el XML como base de captura.';
                if (review) review.textContent = 'XML';
            } else {
                if (label) label.textContent = 'Omitir clonado';
                if (text) text.textContent = 'Continuarás capturando la factura paso a paso.';
                if (review) review.textContent = 'Omitir';
            }
        }

        if (useBtn) useBtn.addEventListener('click', () => apply('clone'));
        if (skipBtn) skipBtn.addEventListener('click', () => apply('skip'));
    ...
    function syncSummary() {
        ...
        const pdf = qs('#manual_pdf');
        const xml = qs('#manual_xml');
        const method = qs('#invx_metodo_pago');
        const complemento = qs('#invx_complemento');
        const status = qs('#manual_status');

        const filesCount = ((pdf && pdf.files && pdf.files.length) ? 1 : 0) + ((xml && xml.files && xml.files.length) ? 1 : 0);

        const map = {
            '#invxSummaryCuenta': account?.value?.trim() || '—',
            '#invxSummaryMonto': formatMoney(amount?.value || '0'),
            '#invxSummaryPeriodo': period?.value?.trim() || '—',
            '#invxSummaryAdjuntos': String(filesCount),
            '#invxSummaryMetodo': optionText(method),
            '#invxSummaryComplemento': optionText(complemento),

            '#invxReviewCuenta': account?.value?.trim() || '—',
            '#invxReviewPeriodo': period?.value?.trim() || '—',
            '#invxReviewMonto': formatMoney(amount?.value || '0'),
            '#invxReviewEstado': optionText(status),
            '#invxReviewMetodo': optionText(method),
            '#invxReviewForma': optionText(qs('#invx_forma_pago')),
            '#invxReviewMoneda': optionText(qs('#invx_moneda')),
            '#invxReviewAdjuntos': String(filesCount)
        };
        ...
        const methodText = optionText(method).toUpperCase();
        const ppdBox = qs('#invxPpdAlertBox');
        const aiBox = qs('#invxAiPreflightBox');

        if (ppdBox) {
            ppdBox.classList.toggle('is-visible', methodText.includes('PPD'));
        }

        if (aiBox) {
            if (methodText.includes('PPD')) {
                aiBox.innerHTML = '<div><strong>IA:</strong> Detecté un posible flujo PPD. Después de emitir, esta factura deberá poder administrar pagos, parcialidades y complementos.</div>';
            } else if (methodText.includes('PUE')) {
                aiBox.innerHTML = '<div><strong>IA:</strong> Detecté flujo PUE. Normalmente el pago se liquida en una sola exhibición.</div>';
            } else {
                aiBox.innerHTML = '<div>La IA revisará la consistencia básica del formulario antes de generar la factura.</div>';
            }
        }
    }

    function bindAssistButtons() {
        const todayBtn = qs('#invxAssistToday');
        const periodBtn = qs('#invxAssistCurrentPeriod');
        const sourceBtn = qs('#invxAssistDefaultSource');
        const clearUuidBtn = qs('#invxAssistClearUuid');
        const aiResponse = qs('#invxAiResponseBox');
        ...
        if (todayBtn) {
            todayBtn.addEventListener('click', function () {
                ...
                const issuedDate = qs('#manual_issued_date');
                const issuedAt = qs('#manual_issued_at');

                if (issuedDate) issuedDate.value = `${yyyy}-${mm}-${dd}`;
                if (issuedAt) issuedAt.value = `${yyyy}-${mm}-${dd}T${hh}:${mi}`;

                if (aiResponse) aiResponse.innerHTML = '<div><strong>IA:</strong> Ya coloqué la fecha actual en los campos de emisión.</div>';
                syncSummary();
            });
        }
        ...
        if (periodBtn) {
            periodBtn.addEventListener('click', function () {
                ...
                if (period) period.value = `${yyyy}-${mm}`;
                if (aiResponse) aiResponse.innerHTML = '<div><strong>IA:</strong> Ya coloqué el periodo actual.</div>';
                syncSummary();
            });
        }
        ...
        if (sourceBtn) {
            sourceBtn.addEventListener('click', function () {
                const source = qs('#manual_source');
                if (source) source.value = 'ia_admin_guided';
                if (aiResponse) aiResponse.innerHTML = '<div><strong>IA:</strong> Marqué el origen como ia_admin_guided.</div>';
            });
        }

        if (clearUuidBtn) {
            clearUuidBtn.addEventListener('click', function () {
                const uuid = qs('#manual_cfdi_uuid');
                if (uuid) uuid.value = '';
                if (aiResponse) aiResponse.innerHTML = '<div><strong>IA:</strong> Limpié el UUID relacionado.</div>';
            });
        }
    }

    function bindAiPanel() {
        const prompt = qs('#invxAiPrompt');
        const response = qs('#invxAiResponseBox');
        const topOpen = qs('#invxOpenAiModalBtn');
        const explainBtn = qs('#invxAiExplainStepBtn');
        const reviewBtn = qs('#invxAiReviewBtn');
        const composeBtn = qs('#invxAiComposeBtn');

        if (topOpen) {
            topOpen.addEventListener('click', function () {
                openModal('invxAiModal');
            });
        }

        if (explainBtn && response) {
            explainBtn.addEventListener('click', function () {
                const msg = STEP_META[currentStep]?.sub || 'Paso actual';
                response.innerHTML = '<div><strong>IA:</strong> ' + msg + '</div>';
            });
        }

        if (reviewBtn && response) {
            reviewBtn.addEventListener('click', function () {
                const account = qs('#manual_account_id')?.value?.trim() || '';
                const amount = qs('#manual_amount_mxn')?.value?.trim() || '';
                const emisor = qs('#invx_emisor_id')?.value?.trim() || '';
                const receptor = qs('#invx_receptor_id')?.value?.trim() || '';
                const issues = [];

                if (!account) issues.push('Falta la cuenta.');
                if (!amount) issues.push('Falta el monto.');
                if (!emisor) issues.push('Falta seleccionar emisor.');
                if (!receptor) issues.push('Falta seleccionar receptor.');

                if (issues.length) {
                    response.innerHTML = '<div><strong>IA:</strong> Revisé el formulario y encontré:</div><div>• ' + issues.join('<br>• ') + '</div>';
                } else {
                    response.innerHTML = '<div><strong>IA:</strong> La captura base se ve consistente para continuar al siguiente paso.</div>';
                }
            });
        }

        if (composeBtn && response) {
            composeBtn.addEventListener('click', function () {
                const text = (prompt?.value || '').trim().toLowerCase();

                if (!text) {
                    response.innerHTML = '<div><strong>IA:</strong> Escribe una descripción primero para ayudarte a completar la factura.</div>';
                    return;
                }
                ...
                if (text.includes('ingreso') && tipo) tipo.value = 'I';
                if (text.includes('egreso') && tipo) tipo.value = 'E';
                if ((text.includes('complemento') || text.includes('pago')) && tipo) tipo.value = 'P';

                if (text.includes('ppd') && method) {
                    const opt = Array.from(method.options || []).find(o => (o.text || '').toUpperCase().includes('PPD'));
                    if (opt) method.value = opt.value;
                }
                ...
                response.innerHTML = '<div><strong>IA:</strong> Apliqué una configuración inicial basada en tu descripción. Revisa los datos y continúa con el wizard.</div>';
            });
        }
    }
