{{-- C:\wamp64\www\pactopia360_erp\resources\views\cliente\sat\external\register.blade.php --}}
{{-- P360 · SAT Registro Externo · UI estilo Login (Light) · v3.0 --}}
@php
  ...
  $success = (bool)($success ?? session('ext_reg_success', false) ?: (request()->query('ok') == '1'));
  $saved   = $saved   ?? session('ext_reg_saved', null);

  if (!$saved && request()->filled('rid')) {
    $saved = [
      'id' => (string) request()->query('rid'),
      'rfc' => old('rfc') ?: null,
    ];
  }
...
<head>
  ...
  <title>Pactopia360 · Registro externo SAT</title>
  ...
  <style>
    ...
    /* Card principal centrada estilo login */
    .auth-shell{
      width:100%;
      max-width: var(--max);
  ...
  <div class="bg">
    <div class="auth-shell" role="main">
      <div class="auth-grid">
        ...
        {{-- LEFT: branding (como login) --}}
        <section class="left" aria-label="Branding">
          <div class="logo-row">
            <img src="{{ $logoUrl }}" alt="PACTOPIA 360">
          </div>

          <h2>Registro seguro para descargas SAT</h2>

          <ul class="bullets">
            <li><b>CFDI 4.0</b> sin fricción con validaciones claras.</li>
            <li><b>Conexión ágil</b> para descargas masivas (XML/PDF).</li>
            <li><b>Bóveda fiscal</b> para resguardo y control documental.</li>
            <li><b>Seguridad reforzada</b> por liga firmada y uso único.</li>
          </ul>
        ...
        {{-- RIGHT: formulario (como login) --}}
        <section class="right" aria-label="Formulario">
          <h1>Registrar RFC</h1>
          <div class="sub">Completa RFC, sube <b>.cer</b> y <b>.key</b> e ingresa la contraseña.</div>
          ...
          @if($success)
            <div class="alert alert-success">
              ...
              @if(is_array($saved) && !empty($saved['id']))
                ID: <b>{{ $saved['id'] }}</b>
              @endif
              @if(is_array($saved) && !empty($saved['rfc']))
                <span style="display:block;margin-top:4px;">RFC: <b>{{ $saved['rfc'] }}</b></span>
              @endif
              @if(is_array($saved) && !empty($saved['soft_mode']))
                <span style="display:block;margin-top:4px;">Nota: validación local en modo compatibilidad (OpenSSL CLI no disponible en Windows).</span>
              @endif
            </div>
          @endif
          ...
          @if($firstError)
            <div class="alert alert-danger">
              <b>No se pudo registrar:</b> {{ $firstError }}
            </div>
          @endif
          ...
          <form class="form" method="POST" action="{{ $actionUrl }}" enctype="multipart/form-data" novalidate>
            ...
            <div class="field">
              <div class="label">
                <span>RFC</span>
                <span class="hint">13 caracteres, mayúsculas</span>
              </div>
              <input type="text" name="rfc" maxlength="13" value="{{ old('rfc') }}" placeholder="AAA010101AAA" required>
            ...
            <div class="field">
              ...
              <input type="text" name="razon_social" maxlength="190" value="{{ old('razon_social') }}" placeholder="Empresa / Persona física">
            ...
            <div class="field">
              <div class="label">
                ...
                <span class="hint">Archivo</span>
              </div>
              <input type="file" name="cer" accept=".cer" required>
              @if($bag?->has('cer')) <div class="field-error">{{ $bag->first('cer') }}</div> @endif
            </div>
            ...
            <div class="field">
              ...
              @if($bag?->has('key')) <div class="field-error">{{ $bag->first('key') }}</div> @endif
            </div>
            ...
            <div class="field">
              ...
              <textarea name="note" rows="2" maxlength="500" placeholder="Ej. Registro para CFDI 2025 / Operación X">{{ old('note') }}</textarea>
            ...
            <label class="check">
              ...
              <div>
                <strong>Confirmo que cuento con autorización para registrar este RFC y cargar su e.firma.</strong>
  ...
  (function () {
    try {
      setTimeout(function () {
        ...
        // Si no se pudo cerrar (no fue opened by script), fallback:
        setTimeout(function () {
