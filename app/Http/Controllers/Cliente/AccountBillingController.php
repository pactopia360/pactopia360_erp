...
final class AccountBillingController extends Controller
{
    ...
    public function statement(Request $r)
    {
        ...
        // ✅ DOMINIO DEL ESPEJO ADMIN:
        // si statements ya trae un periodo pagado más reciente que payments/meta,
        // ese debe mandar en cliente.
        if (
            $this->isValidPeriod((string) $lastPaidFromStatements) &&
            (
                !$this->isValidPeriod((string) $lastPaid) ||
                strcmp((string) $lastPaidFromStatements, (string) $lastPaid) > 0
            )
        ) {
            $lastPaid = (string) $lastPaidFromStatements;
        }
        ...
        // ==========================================================
        // ✅ PRECIO auxiliar por periodo
        // Solo para fallback visual del periodo permitido, NO para rehacer statements.
        // ==========================================================
        ...
        // - NO depender solo de billing_statements
        // - mezclar periods reales desde:
        //   1) billing_statements
        //   2) admin.payments
        //   3) admin.estados_cuenta
        // - así cliente refleja admin en vivo inmediatamente
        // ==========================================================
        $rows = $this->buildLiveAdminMirrorRows(
            $accountId,
            $rowsFromStatementsAll,
            $lastPaid,
            $payAllowedUi,
        ...
        if (empty($rows)) {
            ...
            foreach ($rows as &$row) {
                ...
                $row['price_source']     = $sourcesByPeriod[(string) ($row['period'] ?? '')] ?? ($row['price_source'] ?? 'fallback.clientes');
            }
            unset($row);
        }
   ...
   private function buildLiveAdminMirrorRows(
    ...
    array $chargesByPeriod = []
        ...
        // ======================================================
        // 3) admin.estados_cuenta = FUENTE PRINCIPAL DEL ESPEJO
        // ======================================================
        try {
            if (Schema::connection($adm)->hasTable('estados_cuenta')) {
                $edoCols = Schema::connection($adm)->getColumnListing('estados_cuenta');
                $edoLc   = array_map('strtolower', $edoCols);
                $edoHas  = fn (string $c) => in_array(strtolower($c), $edoLc, true);

                $select = 'periodo, SUM(COALESCE(cargo,0)) as cargo_sum, SUM(COALESCE(abono,0)) as abono_sum';
                if ($edoHas('saldo')) {
                    $select .= ', SUM(COALESCE(saldo,0)) as saldo_sum';
                }
                ...
                foreach ($edoRows as $er) {
                    ...
                    $saldo = property_exists($er, 'saldo_sum')
                        ? round(max(0.0, (float) ($er->saldo_sum ?? 0)), 2)
                        : round(max(0.0, $cargo - $abono), 2);
        ...
        foreach ($periods as $period) {
            ...
            foreach (['total_cargo', 'charge', 'cargo', 'total'] as $k) {
                if (isset($seed[$k]) && is_numeric($seed[$k])) {
                    $statementCharge = round(max(0.0, (float) $seed[$k]), 2);
                    break;
                }
            }
            ...
            foreach (['total_abono', 'paid_amount', 'abono', 'paid'] as $k) {
                if (isset($seed[$k]) && is_numeric($seed[$k])) {
                    $statementPaid = round(max(0.0, (float) $seed[$k]), 2);
                    break;
                }
            }

            $statementSaldo = isset($seed['saldo']) && is_numeric($seed['saldo'])
                ? round(max(0.0, (float) $seed['saldo']), 2)
                : round(max(0.0, $statementCharge - $statementPaid), 2);

            // ======================================================
            // service_items reales
            // ======================================================
            ...
            foreach ($serviceItems as $it) {
                ...
                // ignora fallback
                if (strtolower((string) data_get($meta, 'source', '')) === 'fallback') {
                    continue;
                }
            ...
            $edoSaldo  = round((float) ($edoAgg[$period]['saldo'] ?? max(0.0, $edoCharge - $edoPaid)), 2);
            ...
            // ======================================================
            // REGLA NUEVA:
            // admin.estados_cuenta manda si existe para ese periodo
            // ======================================================
            $charge = 0.0;
            $paidAmount = 0.0;
            $saldo = 0.0;
            ...
            if ($edoCharge > 0.0001 || $edoPaid > 0.0001 || $edoSaldo > 0.0001) {
                $charge = $edoCharge > 0.0001
                    ? $edoCharge
                    : max($realItemsTotal, $statementCharge, $monthlyCharge);

                $paidAmount = $edoPaid > 0.0001
                    ? $edoPaid
                    : max($statementPaid, $paymentsPaid);

                $saldo = $edoSaldo > 0.0001
                    ? $edoSaldo
                ...
                if ($saldo <= 0.0001 && $charge > 0.0001) {
                    ...
                    $saldo = 0.0;
                } elseif ($paidAmount > 0.0001 && $saldo > 0.0001) {
                    $status = 'partial';
                } elseif ($charge > 0.0001) {
                    $status = 'pending';
                } else {
                    $status = 'no_movement';
                }
            } else {
                // fallback si admin.estados_cuenta no tiene nada
                $charge = $realItemsTotal > 0.0001
                    ? $realItemsTotal
                    : ($statementCharge > 0.0001
                        ? $statementCharge
                        : $monthlyCharge);

                $paidAmount = max($statementPaid, $paymentsPaid);
                $saldo = $statementSaldo > 0.0001
                    ? $statementSaldo
                    : round(max(0.0, $charge - $paidAmount), 2);
            ...
            // overrides solo ajustan status final
            if ($ov && !empty($ov['status_override'])) {
                ...
                if ($ovStatus === 'pagado') {
                    ...
                    if ($charge > 0.0001 && $saldo <= 0.0001) {
                        $saldo = round(max(0.0, $charge - $paidAmount), 2);
                    }
                    $status = $saldo > 0.0001 ? 'partial' : 'paid';
            ...
            $paidAmount = round(max(0.0, min($paidAmount, $charge > 0 ? $charge : $paidAmount)), 2);
            ...
            $out[] = [
                ...
                'price_source'           => ($edoCharge > 0.0001 || $edoPaid > 0.0001 || $edoSaldo > 0.0001)
                    ? 'admin.estados_cuenta.sot'
                    : ($realItemsTotal > 0.0001
   ...
   /**
    ...
    /**
     * ==========================================================
     * ✅ RENDER PDF usando el MISMO espejo vivo que la pantalla web
     * ==========================================================
     */
    private function renderStatementPdf(\Illuminate\Http\Request $r, int $accountId, string $period, bool $inline)
    {
        ...
        try {
            $rowsAll = $this->applyStatementOverridesForAdminAccount($accountId, $rowsAll);
        } catch (\Throwable $e) {
            // continúa sin overrides
        }
        ...
        $basePeriod = $this->resolveContractStartPeriod($accountId);
        $basePeriod = $this->isValidPeriod((string) $basePeriod)
            ? (string) $basePeriod
            : now()->format('Y-m');
        ...
        $payAllowedUi = is_string($payAllowed) && $this->isValidPeriod($payAllowed)
            ? $payAllowed
            : $period;
        ...
        foreach ((array) $rowsAll as $rr) {
            ...
            $charge = 0.0;
            foreach (['total_cargo', 'charge', 'cargo', 'total'] as $k) {
                if (isset($rr[$k]) && is_numeric($rr[$k])) {
                    $charge = round(max(0.0, (float) $rr[$k]), 2);
                    break;
                }
            }

            if ($charge > 0.0001) {
                $chargesByPeriod[$p] = $charge;
            }
        }

        $mirrorRows = $this->buildLiveAdminMirrorRows(
            $accountId,
            (array) $rowsAll,
            $lastPaid,
            $payAllowedUi,
        ...
        $mirrorRows = $this->applyAdminPaidAmountOverrides($accountId, $mirrorRows);

        $row = null;
        foreach ($mirrorRows as $mr) {
            if ((string) ($mr['period'] ?? '') === $period) {
                $row = $mr;
                break;
            }
        }
        ...
        if (!$row) {
            Log::warning('[BILLING][PDF] period not found in live mirror', [
                'account_id' => $accountId,
                'period'     => $period,
                'periods'    => array_map(fn ($x) => $x['period'] ?? null, $mirrorRows),
            ]);
        ...
        foreach ($mirrorRows as $mr) {
            ...
            $st = $this->normalizeStatementStatus((string) ($mr['status'] ?? 'pending'));
            $saldoRow = isset($mr['saldo']) && is_numeric($mr['saldo'])
                ? round(max(0.0, (float) $mr['saldo']), 2)
                : 0.0;

            // ✅ CLAVE: si el espejo vivo dice pagado o saldo 0, NO entra al PDF.
            ...
            $serviceItems[] = [
                'name'       => ($isAnnual ? 'Suscripción anual ' : 'Mensualidad ') . $label,
                'unit_price' => $saldoRow,
                'qty'        => 1,
                'subtotal'   => $saldoRow,
                'period'     => $rp,
        ...
        foreach (['total_cargo', 'charge', 'cargo', 'total'] as $k) {
            if (isset($row[$k]) && is_numeric($row[$k])) {
                $cargo = round(max(0.0, (float) $row[$k]), 2);
                break;
            }
        }
        ...
        foreach (['total_abono', 'paid_amount', 'abono', 'paid'] as $k) {
            if (isset($row[$k]) && is_numeric($row[$k])) {
                $abono = round(max(0.0, (float) $row[$k]), 2);
                break;
            }
        }
   ...
   private function loadInvoiceStatusMapFromTable(string $adm, string $table, int $accountId, array $periods): array
    ...
    private function renderSimplePdfHtml(array $d): string
    {
        ...
        /**
     * ==========================
     * DESCARGAR ZIP DE FACTURA
     * ==========================
     * SOT:
     * - admin.billing_invoice_requests
     * - fallback: admin.invoice_requests
     *
     * Reglas:
     * - solo descarga facturas de la cuenta autenticada
     * - busca el request más reciente del periodo
