...
final class SatExternalPublicController extends Controller
{
    ...
    public function externalInviteFallback(Request $request): JsonResponse
    {
        ...
        if ($v->fails()) {
            if ($viewName && !$request->expectsJson()) {
        ...
        // 72h: URL firmada con email + cuenta
        // Importante: aquí mandamos cuenta_id (para que quede alineado con lo que ya estás usando)
     ...
     */
    public function externalRegisterForm(Request $request): Response
    {
    ...
    public function externalRegisterStore(Request $request)
    {
        ...
        if (!$cuentaId) {
            ...
            try { \Log::warning('p360_sat_external_register_no_account', ['email' => $email, 'ip' => $request->ip()]); } catch (\Throwable) {}
        ...
        if ($v->fails()) {
            try { \Log::info('p360_sat_external_register_validation_failed', ['email'=>$email,'cuenta_id'=>$cuentaId,'first'=>$v->errors()->first()]); } catch (\Throwable) {}
        ...
        if (!$okCsd && app()->environment('local')) {
            ...
            if ($noCli) {
                ...
                try { \Log::warning('p360_sat_external_register_soft_mode_local', ['email'=>$email,'cuenta_id'=>$cuentaId,'rfc'=>$rfc,'reason'=>'openssl_cli_missing_windows']); } catch (\Throwable) {}
            }
        }
        ...
        if (!$okCsd) {
            ...
            try { \Log::info('p360_sat_external_register_csd_failed', ['email'=>$email,'cuenta_id'=>$cuentaId,'rfc'=>$rfc,'msg'=>$msg,'details'=>$csdDetails]); } catch (\Throwable) {}
        ...
        try { $enc = encrypt($pass); }
        catch (\Throwable) { $enc = base64_encode($pass); }
    ...
    private function validateCsdPair(string $cerPath, string $keyPath, string $password): array
    {
        ...
        if ($keyPem === null) {
            // Si el problema es que NO hay OpenSSL CLI (común en Windows), damos mensaje claro.
            $cliErr = $details['key']['convert_error'] ?? '';
            $noCli  = is_string($cliErr) && str_contains($cliErr, 'OpenSSL CLI no está disponible');

            if ($noCli) {
                // ✅ Modo local: no bloqueamos guardado. La validación real se hará al intentar conectar al SAT en servidor que sí tenga openssl.
                // Si quieres forzarlo SIEMPRE, quita este bloque y deja el return false.
                $details['key']['soft_mode'] = true;
        ...
        if ($priv === false) {
            // Intento: sin password por si viene sin cifrar (no debería, pero pasa)
        ...
        // Comparación robusta:
        // - RSA: compara modulus
        // - Otros: compara representación de public key (fallback)
        $match = false;
        ...
        if (
            ...
            $privPub = $privDet['key'] ?? null; // public part (PEM) derivada
            ...
            if (is_string($privPub) && is_string($certPub) && $privPub !== '' && $certPub !== '') {
                ...
                // Último fallback: firma/verifica con el par (no perfecto pero práctico)
    ...
    private function normalizePrivateKeyToPem(string $raw, string $password, string $keyPath, array &$details): ?string
    {
        ...
        // 2) Probable DER binario (SAT .key)
        ...
        // 3) Intentar convertir DER->PEM usando openssl CLI (si existe)
        ...
        // 4) Si NO hay CLI (Windows típico), dejamos evidencia clara y regresamos null
        //    (La validación CSD fallará con mensaje explícito; puedes decidir permitir "soft" en local)
        return null;
    }
    ...
    private function convertDerKeyToPemUsingOpenssl(string $keyPath, string $password, array &$details): ?string
    {
        ...
        // En Windows, "openssl" puede no estar en PATH. Detectamos primero.
 ...
 */
public function externalInviteGet(Request $request): Response
{
    // Esta pantalla es interna (requiere login por estar dentro del grupo sat)
    // No removemos middlewares aquí.
